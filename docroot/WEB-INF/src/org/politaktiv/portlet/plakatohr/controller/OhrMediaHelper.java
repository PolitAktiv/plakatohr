...
/**
 * Helper class for all kinds of interaction with LifeRay's file/media library.
 * Needs to be instantiated so that the logging works.
 */
public class OhrMediaHelper {
	...
	/**
	 * Retrieves a list of file entries that serve as previews for the
	 * background images from the source folder. This will only return file
	 * entries for which both a JPG and SVG version exist. JPGs and SVGs are
	 * determined by their MIME type. SVGs are looked up from the JPGs by
	 * replacing the file extension (if any) by ".svg". The comparison is
	 * case-insensitive. Any errors are logged.
	 * 
	 * @param folderId
	 *            the source folder ID
	 * @param themeDisplay
	 *            used by backend methods.
	...
	/**
	 * Retrieves a map of file entries that serve as previews for the background
	 * images from the source folder – each preview entry is mapped to an SVG
	 * template entry. This will only return file entries for which both a JPG
	 * and SVG version exist. JPGs and SVGs are determined by their MIME type.
	 * SVGs are looked up from the JPGs by replacing the file extension (if any)
	 * by ".svg". The comparison is case-insensitive. Any errors are logged.
	 * 
	 * @param folderId
	 *            the source folder ID
	 * @param themeDisplay
	 *            used by backend methods.
	 * @return a mapping from preview entries (JPG) to template entries (SVG) or
	...
	public Map<DLFileEntry, DLFileEntry> getBackgroundPreviewsAndTemplates(long folderId, ThemeDisplay themeDisplay) {
		...
		try {
			dlFileEntries = DLFileEntryLocalServiceUtil.getFileEntries(themeDisplay.getScopeGroupId(),
					dir.getFolderId());
	 ...
	 * 
	 * @param folderId
	 *            the folder ID to translate.
	 * @return the folder name or null in any case of error (errors are logged
	 *         in detail).
	 */
	...
	/**
	 * Retrieves a folder object by its ID. Errors will yield null, details
	 * being logged.
	 * 
	 * @param folderId
	 *            the ID of the folder to revtrieve
	...
	public boolean userHasWritePermission(ThemeDisplay themeDisplay, DLFolder folder) {
		...
		/*
		 * // see
		 * https://github.com/liferay/liferay-portal/blob/master/portal-impl/src
		 * /com/liferay/portlet/documentlibrary/service/permission/
		 * DLFolderPermission.java String portletId =
		 * PortletProviderUtil.getPortletId( Folder.class.getName(),
		 * PortletProvider.Action.EDIT);
		 */

		Boolean check = StagingPermissionUtil.hasPermission(permissionChecker, folder.getGroupId(),
				folder.getClass().getName(), folder.getFolderId(), portletId, ActionKeys.ADD_DOCUMENT);
	 ...
	 * 
	 * @param themeDisplay
	 *            needed by service methods.
	 * @param fileEntry
	 *            the file entry to find the URL for
	 * @return the public URL for that file entry or null in any case of error
	...
	/**
	 * Uploads a file into a folder. File contents is taken from a stream. File
	 * titles must be unique in that folder, otherwise an exception will be
	 * thrown.
	 * 
	 * @param targetFolderId
	 *            the folder to upload data to.
	 * @param mimeType
	 *            the mime type of the new file.
	 * @param title
	 *            the title (displayed as name) of the new file.
	 * @param is
	 ...
	 */
	@Deprecated
	public void storeFile(long targetFolderId, String mimeType, String title, InputStream is, PortletRequest request,
	 ...
	 *             on any kind of error.
	 */
	public long storeFile(long targetFolderId, String mimeType, String title, InputStream is, PortletRequest request)
			throws IOException {
		...
		try {
			...
			// TODO: file name null geht irgendwie nicht gut? Preview generator
			// macht Quatsch!
			FileEntry file = DLAppServiceUtil.addFileEntry(getThemeDisplay(request).getScopeGroupId(), targetFolderId,
					null, mimeType, title, "", "", bufferIs, size, serviceContext);
			id = file.getFileEntryId();
			setFilePermissions(file, RoleConstants.GUEST, ActionKeys.VIEW);
			setFilePermissions(file, RoleConstants.USER, ActionKeys.VIEW);
		} catch (Exception e) {
			_log.error(e);
	...
	public static void setFilePermissions(FileEntry fileEntry, String roleName, String permission) throws Exception {
		ResourcePermission resourcePermission = null;
		final Role siteMemberRole = RoleLocalServiceUtil.getRole(fileEntry.getCompanyId(), roleName);
